...
capacidade Total+=caminhoes [linha][0];
...
printf("\nProducao: %.21f", producao);

double custoTransp;

for(linha=0; linha<qtdeCaminhao; linha++){

printf("[%lf] [%lf] [%1f]", caminhoes [linha] [0],caminhoes [linha] [1], cami nhoes [linha] [2]);

if((caminhoes[linha][0])>=45) {

custoTransp=((caminhoes [linha] [e]) (caminhoes [linha][1]))*((caminhoes [linha] [2])*0.75)

}else{ custoTransp=((caminhoes [linha][0])* (caminhoes [linha] [1]))*(caminhoes [linha] [2]); }
